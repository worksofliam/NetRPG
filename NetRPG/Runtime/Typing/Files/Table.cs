...
namespace NetRPG.Runtime.Typing.Files
{
    class Table : FileT
    {
        ...
        public Table(string name, string file, bool userOpen) : base(name, file, userOpen) {
            this.Name = name;
            _Path = Path.Combine(Environment.CurrentDirectory, "QSYS.LIB/test.lib", file + ".json");
        ...
        public static DataSet CreateStruct(string name, Boolean qualified = false) {
                string content = File.ReadAllText(Path.Combine(Environment.CurrentDirectory, "QSYS.LIB/test.lib", name + ".json"));
